...
def _pgm_pixels(data: bytes) -> tuple[int, int, bytes]:
    """Width, height and raw 8-bit pixels of a binary grayscale PGM (the simplest image format there is)."""
    assert data[:2] == b"P5", "expected a binary PGM from pdftoppm -gray"
    values, position = [], 2
    while len(values) < 3:  # width, height, maxval, separated by whitespace and possibly comments
        ...
        while not data[position : position + 1].isspace():
            position += 1
        values.append(int(data[begin:position]))
    width, height, maxval = values
    assert maxval == 255
    pixels = data[position + 1 :]  # exactly one whitespace byte separates the header from the pixels
...
def scanned_pdf(pages: list[str], *, dpi: int = 150, size: int = 24) -> bytes:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for n in range(1, len(pages) + 1):
            out = Path(tmp) / f"p{n}"
            cmd = ["pdftoppm", "-r", str(dpi), "-gray", "-singlefile", "-f", str(n), "-l", str(n)]
            subprocess.run(cmd + [str(source), str(out)], check=True)
            images.append(_pgm_pixels(Path(f"{out}.pgm").read_bytes()))
    ...
    for i, (width, height, raw) in enumerate(images):
        pixels = zlib.compress(raw)
